...
from edu.gui.helpers import delete_project, list_projects, list_projects_json, load_project, show_config, init
...
async def start_new_lesson(sm: StateManager, ui: UIBase) -> bool:
    """
    Start a new lesson.

    :param sm: State manager.
    :param ui: User interface.
    :return: True if the lesson was created successfully, False otherwise.
    """
    while True:
        try:
            user_input = await ui.ask_question(
                "Enter a name for your workspace directory: ",
                allow_empty=False,
                source=pythagora_source,
            )
        except (KeyboardInterrupt, UIClosedError):
        ...
        lesson_topic = user_input.text.strip()
        if not lesson_topic:
            await ui.send_message("Please choose a lesson topic", source=pythagora_source)
        elif len(lesson_topic) > 100:
            await ui.send_message("Please choose a shorter lesson topic", source=pythagora_source)
        else:
            break

    # Here, you would typically create a new lesson state instead of a project
    # For now, we'll just use the create_project method
    lesson_state = await sm.create_project(lesson_topic)
    return lesson_state is not None
...
async def run_pythagora_session(sm: StateManager, ui: UIBase, args: Namespace):
    ...
    if args.project or args.branch or args.step:
        ...
        success = await start_new_lesson(sm, ui)
        if not success:
            return False
...
async def tkinter_main(ui: UIBase, db, args):
    log.debug(f"UI type in tkinter_main: {type(ui).__name__}")
    try:
...
def run_pythagora():
    ui, db, args = init()
    log.debug(f"UI type in run_pythagora: {type(ui).__name__}")
    if not ui or not db:
        return -1

    async def run_and_update():
        await ui.start()  # Start the UI
        task = asyncio.create_task(tkinter_main(ui, db, args))
        while not task.done():
            ui.update()
            await asyncio.sleep(0.1)
        return await task

    success = asyncio.run(run_and_update())
    return 0 if success else -1


def parse_arguments() -> Namespace:
    # Implement your argument parsing logic here
    # This should return a Namespace object with all the necessary arguments
    pass
